	...
	for (n=1; n<1100000; n*=2) {
		...
		for (k=0; k<NUMBER_OF_TESTS; k++) {
			if (rank == 0) {
				...
				int dataOut = 13, dataIn;
				requestList = (MPI_Request*)malloc((nloop-1)*sizeof(MPI_Request));

				for (j=0; j<nloop; j++) {
					MPI_Isend(buf, n, MPI_DOUBLE, 1, k, MPI_COMM_WORLD, &requestNull);
				}

				for (j=0; j<nloop; j++) {
					MPI_Irecv(buf, n, MPI_DOUBLE, 1, k, MPI_COMM_WORLD, &(requestList[j-1]));
				}

				for (j=0; j<nloop; j++) {
					int index;	
					MPI_Waitany( nloop - 1, requestList, &index, &status);
